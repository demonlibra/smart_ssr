...
# --- Как пользоваться? --- #
	...
	#	 !!!Не изменяейте параметры, влияющие на перемещения осей и длину выдавливаемого филамента.
	# 2. Проверьте параметры ниже.
	# 3. Запустите сценарий в терминале командой "bash retract_calibration_towers.sh".
