...
// Версия 3.3 для Arduino от 14/08/2020
...
float duty_cycle;                     // Коэффициент заполнения ШИМ от платы управления принтером
float radian;
...
void loop() {
  ...
  duty_cycle = (float)duty / period_pwm;                    // Rоэффициент заполнения ШИМ = Длительность импульса / период (20000 мкс)
  ...
  // Необходимая площадь S = (Площадь полупериода синусоиды) х (Коэффициент заполнения) = duty_cycle x 2
  //
